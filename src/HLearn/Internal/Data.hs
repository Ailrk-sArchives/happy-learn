...
-- | Used to index.
data Point a = Point
  { unPoint :: {-# UNPACK #-} !(UV.Vector a) -- coordinate of the point.
  }
...
inBoundCheck :: (UV.Unbox a, Ord a, Num a, R.Shape sh) => sh -> Point a -> Bool
...
validatePoint ::
  (UV.Unbox a, Num a, Ord a, R.Shape sh) =>
  sh ->
  Point a ->
...
validatePoint sh p@(Point vs)
  ...
  | otherwise = Right $ p
  where
    shapeMismatch = R.rank sh /= UV.length vs
...
makePointPair ::
  (R.Shape sh, UV.Unbox a, Ord a, Num a) =>
  sh ->
  Point a ->
  Point a ->
...
makePointPair sh as@(Point ad) bs@(Point bd)
  ...
  where
    ...
    shapeMismatch = UV.length ad /= UV.length bd
    outOfBound = all (== True) (not . inBoundCheck sh <$> [as, bs])

makePoint ::
  (UV.Unbox a, Num a, Ord a, R.Shape sh) => sh -> UV.Vector a -> Either InternalError (Point a)
makePoint sh = validatePoint sh . Point

-- | Make zero point.
zeroPoint :: (UV.Unbox a, Fractional a) => Int -> Point a
zeroPoint dim =
  Point
    { unPoint = UV.replicate dim 0
    }
{-# INLINE zeroPoint #-}

-- | Return Right if all elements in the list have the same shape
makeNonEmptyPointList :: UV.Unbox a => [Point a] -> Either InternalError (NonEmptyPointList a)
makeNonEmptyPointList [] = Left $ DataError "PointArray is non empty"
makeNonEmptyPointList xs
  | let dims = (UV.length . unPoint <$> xs)
        x = head dims
...
compactPoints :: (UV.Unbox a) => NonEmptyPointList a -> R.Array R.U R.DIM2 a
compactPoints (NonEmptyPointList t@(x NL.:| _)) = R.fromUnboxed sh unboxedArray
  where
    rank' = UV.length . unPoint $ x
    x' = length t
    sh = R.Z R.:. x' R.:. rank'
    unboxedArray = UV.create $ do
      let idxedList = [0 ..] `zip` (NL.toList t)
          totalSize = length t * UV.length (unPoint x)
      v <- MV.new $ totalSize
      sequence_
        [ MV.write v (i * rank' + j) e
          | (i, pv) <- idxedList,
            (j, e) <- [0 ..] `zip` (UV.toList . unPoint $ pv)
        ]
      return v
